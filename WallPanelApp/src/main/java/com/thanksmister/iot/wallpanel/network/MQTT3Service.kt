...
class MQTT3Service(private var context: Context, options: MQTTOptions,
    ...
    override fun close() {
        ...
        mqtt3AsyncClient?.let {
            ...
            mqttOptions?.let {
                val offlineMessage = Mqtt3Publish.builder().topic("${it.getBaseTopic()}${CONNECTION}").payload(OFFLINE.toByteArray()).retain(true).build()
    ...
    override fun publish(topic: String, payload: String, retain: Boolean) {
        try {
            if (isReady) {
                ...
                mqttOptions?.let {
                    val mqttMessage = Mqtt3Publish.builder().topic(topic).payload(payload.toByteArray()).retain(retain).build()
    ...
    @SuppressLint("NewApi")
    @Throws(Mqtt3MessageException::class, IOException::class, NoSuchAlgorithmException::class, InvalidKeySpecException::class)
        ...
        try {
            mqttOptions?.let { mqttOptions ->
                ...
                mqttBuilder.addConnectedListener { context: MqttClientConnectedContext? ->
                    ...
                    val onlineMessage = Mqtt3Publish.builder().topic("${mqttOptions.getBaseTopic()}${CONNECTION}").payload(ONLINE.toByteArray()).retain(true).build()
